...
void PlotEvents(std::vector<TString> filesin, std::vector<bool> if_event_exist, std::vector<int> sample_types, string plot_prefix, string cut, int i_toy_index, int nentries, int firstentry, int ndivisions, string para_name, double RWstart, double RWstep, int RWnstep) {
	...
	TH2F* MvvCosSA_sum_event_weight_temps = new TH2F("MvvCosSA_sum_event_weight_temps", "Event weight ; M_{VV}(M_{4l}) ; cos(#Theta)", 26, 490, 1010, 15, -1.0, 1.0);
	TH2F* MvvCosSA_L_2_temps = new TH2F("MvvCosSA_L_2_temps", "C_{2} ; M_{VV}(M_{4l}) ; cos(#Theta)", 26, 490, 1010, 15, -1.0, 1.0);
	TH2F* MvvCosSA_I_3_temps = new TH2F("MvvCosSA_I_3_temps", "I_{3} ; M_{VV}(M_{4l}) ; cos(#Theta)", 26, 490, 1010, 15, -1.0, 1.0);

	for(int index=1; index<=8; index++) {
		fa_temps[index-1] = 0;
		ga_temps[index-1] = 0;
		MvvCosSA_ph_temps[index-1] = new TH2F(Form("MvvCosSA_ph_temps_%i",index), Form("P%i_tilde_Boson1 ; M_{VV}(M_{4l}) ; cos(#Theta)",index), 26, 490, 1010, 15, -1.0, 1.0);
		MvvCosSA_mh_temps[index-1] = new TH2F(Form("MvvCosSA_mh_temps_%i",index), Form("P%i_tilde_Boson2 ; M_{VV}(M_{4l}) ; cos(#Theta)",index), 26, 490, 1010, 15, -1.0, 1.0);
		for(int index2=1; index2<=8; index2++) {
			hab_temps[index-1][index2-1] = 0;
			MvvCosSA_pmh2d_temps[index-1][index2-1] = new TH2F(Form("MvvCosSA_pmh2d_temps_%i_%i",index,index2), Form("(P%i_tilde_Boson1 #times P%i_tilde_Boson2) ; M_{VV}(M_{4l}) ; cos(#Theta)",index,index2), 26, 490, 1010, 15, -1.0, 1.0);
		}
	}
	...
	for(int fid=0; fid<nfiles; fid++){ 
		...
		chin.Add(filesin.at(fid));
		chin.SetBranchStatus("*", 1);
		chin.Draw(Form("event_weight>>+sum_event_weight_temps"), Form("1.0"), "goff", nentries, firstentry);
		chin.Draw(Form("Cos_Scatter_Angle:M4l>>+MvvCosSA_sum_event_weight_temps"), Form("1.0"), "goff", nentries, firstentry);
		sum_event_weight_temps = (TH1F*)(gDirectory->Get(Form("sum_event_weight_temps")));
		for(int index=1; index<=8; index++) {
			//chin.Draw(Form("%s>>+ph_temps_%i", Form("P%i_plus_Boson1",index), index), Form("1.0*(P%i_plus_Boson1)",index), "goff", nentries, firstentry);
			//chin.Draw(Form("%s>>+mh_temps_%i", Form("P%i_plus_Boson2",index), index), Form("1.0*(P%i_plus_Boson2)",index), "goff", nentries, firstentry);
			chin.Draw(Form("%s>>+ph_temps_%i", Form("P%i_tilde_Boson1",index), index), Form("1.0*(P%i_tilde_Boson1)*0.5",index), "goff", nentries, firstentry);
			chin.Draw(Form("%s>>+mh_temps_%i", Form("P%i_tilde_Boson2",index), index), Form("1.0*(P%i_tilde_Boson2)*0.5",index), "goff", nentries, firstentry);
			chin.Draw(Form("Cos_Scatter_Angle:M4l>>+MvvCosSA_ph_temps_%i", index), Form("1.0*(P%i_tilde_Boson1)*0.5",index), "goff", nentries, firstentry);
			chin.Draw(Form("Cos_Scatter_Angle:M4l>>+MvvCosSA_mh_temps_%i", index), Form("1.0*(P%i_tilde_Boson2)*0.5",index), "goff", nentries, firstentry);
			for(int index2=1; index2<=8; index2++) {
				//chin.Draw(Form("%s:%s>>+pmh2d_temps_%i_%i", Form("P%i_plus_Boson2",index2), Form("P%i_plus_Boson1",index), index, index2), Form("1.0*(P%i_plus_Boson1)*(P%i_plus_Boson2)",index,index2), "goff", nentries, firstentry); // %s:%s is act as y:x, which means P%i_plus_Boson1 in X axis and P%i_plus_Boson2 in Yaxis
				chin.Draw(Form("%s:%s>>+pmh2d_temps_%i_%i", Form("P%i_tilde_Boson2",index2), Form("P%i_tilde_Boson1",index), index, index2), Form("1.0*(P%i_tilde_Boson1)*(P%i_tilde_Boson2)*0.25",index,index2), "goff", nentries, firstentry);
				chin.Draw(Form("Cos_Scatter_Angle:M4l>>+MvvCosSA_pmh2d_temps_%i_%i",index,index2), Form("1.0*(P%i_tilde_Boson1)*(P%i_tilde_Boson2)*0.25",index,index2), "goff", nentries, firstentry);
			}
		}
	}
	...
	TFile MvvCosSA_hists(Form("MvvCosSA_hists_toyindex%i.root", i_toy_index), "RECREATE");
	MvvCosSA_hists.cd();
	for(int index=1; index<=8; index++) {
	...
	ofstream text_ouput_MvvCosSA_ph_temps(Form("./text_ouput_MvvCosSA_fa_toyindex%i.txt", i_toy_index), ios::ate);
	ofstream text_ouput_MvvCosSA_mh_temps(Form("./text_ouput_MvvCosSA_ga_toyindex%i.txt", i_toy_index), ios::ate);
	ofstream text_ouput_MvvCosSA_pmh2d_temps(Form("./text_ouput_MvvCosSA_hab_toyindex%i.txt", i_toy_index), ios::ate);
	...
	for(int xbin=1; xbin<=(MvvCosSA_sum_event_weight_temps->GetXaxis()->GetNbins()); xbin++){
		for(int ybin=1; ybin<=(MvvCosSA_sum_event_weight_temps->GetYaxis()->GetNbins()); ybin++){
			text_ouput_MvvCosSA_ph_temps<<(MvvCosSA_ph_temps[1-1]->GetXaxis()->GetBinCenter(xbin))<<"\t"<<(MvvCosSA_ph_temps[1-1]->GetYaxis()->GetBinCenter(ybin))<<endl;
			text_ouput_MvvCosSA_mh_temps<<(MvvCosSA_mh_temps[1-1]->GetXaxis()->GetBinCenter(xbin))<<"\t"<<(MvvCosSA_mh_temps[1-1]->GetYaxis()->GetBinCenter(ybin))<<endl;
			text_ouput_MvvCosSA_pmh2d_temps<<(MvvCosSA_pmh2d_temps[1-1][1-1]->GetXaxis()->GetBinCenter(xbin))<<"\t"<<(MvvCosSA_pmh2d_temps[1-1][1-1]->GetYaxis()->GetBinCenter(ybin))<<endl;
		...
		//c_hstacks->SaveAs(Form("./plots/P+_%i_toyindex%i.png", index, i_toy_index));
...
///=-		tc_MvvCosSA_ph_temps.SaveAs(Form("tc_%s_toyindex%i.png",MvvCosSA_ph_temps[index-1]->GetName(), i_toy_index));
///=-		TCanvas tc_MvvCosSA_mh_temps(Form("tc_%s",MvvCosSA_mh_temps[index-1]->GetName()),"");
///=-		MvvCosSA_mh_temps[index-1]->Draw("COLZ");
///=-		tc_MvvCosSA_mh_temps.SaveAs(Form("tc_%s_toyindex%i.png",MvvCosSA_mh_temps[index-1]->GetName(), i_toy_index));
///=-		for(int index2=1; index2<=8; index2++) {
...
///=-			tc_MvvCosSA_pmh2d_temps.SaveAs(Form("tc_%s_toyindex%i.png",MvvCosSA_pmh2d_temps[index-1][index2-1]->GetName(), i_toy_index));
...
///=-			//c_hstacks->SaveAs(Form("./plots/P_+%i_-%i_toyindex%i.png", index, index2, i_toy_index));
///=-		}
///=-	}
///=-	TCanvas tc_MvvCosSA_L_2_temps(Form("tc_%s",MvvCosSA_L_2_temps->GetName()),"");
///=-	MvvCosSA_L_2_temps->Draw("COLZ");
///=-	tc_MvvCosSA_L_2_temps.SaveAs(Form("tc_%s_toyindex%i.png",MvvCosSA_L_2_temps->GetName(), i_toy_index));
///=-	TCanvas tc_MvvCosSA_I_3_temps(Form("tc_%s",MvvCosSA_I_3_temps->GetName()),"");
///=-	MvvCosSA_I_3_temps->Draw("COLZ");
///=-	tc_MvvCosSA_I_3_temps.SaveAs(Form("tc_%s_toyindex%i.png",MvvCosSA_I_3_temps->GetName(), i_toy_index));
///=-
///=-
///=-	for(int index=1; index<=8; index++) {
...
///=-				c_hstacks->SaveAs(Form("./plots/P+%i_toyindex%i.png", index, i_toy_index));
...
///=-				temp_h1f_PvsD->Draw("HIST");
///=-				c_hstacks->SaveAs(Form("./plots/P~Boson1%i_toyindex%i.png", index, i_toy_index));
///=-				temp_h1f_PvsD->Reset("ICES M");
///=-				// ------------------------------------------------ 1D
///=-				c_hstacks->Clear();
///=-				c_hstacks->cd();
...
///=-				temp_h1f_PvsD->Draw("HIST");
///=-				c_hstacks->SaveAs(Form("./plots/P~Boson2%i_toyindex%i.png", index, i_toy_index));
///=-				temp_h1f_PvsD->Reset("ICES M");
///=-				// ------------------------------------------------ 2D
///=-				c_hstacks->Clear();
///=-				c_hstacks->cd();
...
///=-				c_hstacks->SaveAs(Form("./plots/P+%i_VS_%s_toyindex%i.png", index, LepDynamicsNmae.c_str(), i_toy_index));
...
///=-				temp_h2f_PvsD->Draw("COLZ");
///=-				c_hstacks->SaveAs(Form("./plots/P~Boson1%i_VS_%s_toyindex%i.png", index, LepDynamicsNmae.c_str(), i_toy_index));
///=-				temp_h2f_PvsD->Reset("ICES M");
///=-				// ------------------------------------------------ 2D
///=-				c_hstacks->Clear();
///=-				c_hstacks->cd();
...
///=-				temp_h2f_PvsD->Draw("COLZ");
///=-				c_hstacks->SaveAs(Form("./plots/P~Boson2%i_VS_%s_toyindex%i.png", index, LepDynamicsNmae.c_str(), i_toy_index));
///=-				temp_h2f_PvsD->Reset("ICES M");
///=-				for(int index2=1; index2<=8; index2++) {
...
///=-					c_hstacks->SaveAs(Form("./plots/P~Boson1%i_VS_P~Boson2%i_VS_%s_toyindex%i.png", index, index2, LepDynamicsNmae.c_str(), i_toy_index));
...
///=-					temp_h3f_PvsD->Draw("COLZ");
///=-					c_hstacks->SaveAs(Form("./plots/P+%i_VS_P-%i_VS_%s_toyindex%i.png", index, index2, LepDynamicsNmae.c_str(), i_toy_index));
///=-					temp_h3f_PvsD->Reset("ICES M");
///=-				}
///=-			}
///=-		}
...
void selection_draw(string para_name, double RWstart, double RWstep, int RWnstep, string hardcut="( (Mrecoil>0 && Mrecoil<200) && (Mll1>80 && Mll1<100) && (Mll2>80 && Mll2<100) && (deltaM<20) )", double CLv=0.95, int TStype=3){
	...
	//--------------------------------------------------------------------------------------------------------------
	Infiles_names.push_back(TString(Form("QE_mumuToZZTollll_SM_1TeVTeV_run_0[12].root"))); Sample_Types.push_back(0);
	...
	// For our use case the "nfiles_" should keep to be 1, which means PlotEvents function only has one input root file. Only by the configuration (I did not think out an alternative way) the setup of parameter "nentries, firstentry" can perform as we expect (i.e. the unmber of events filled into histogram is as expected (L x CrossSection))
        int nfiles_ = Outfiles_names_preselected.size();
	if( nfiles_ != 1 ) return;
	TChain chin_("tree");
	for(int fid_=0; fid_<nfiles_; fid_++){
		chin_.Add(Outfiles_names_preselected.at(fid_));
	}
	int ent_total = chin_.GetEntries();
	// For 1TeV mumuToZZTollll, CX=, having 6888 events under 10ab-1
	int nentries_mean=6888;
	int nentries=nentries_mean;
	int firstentry=0;
	for(int i_toy=0; (firstentry+(7*sqrt(nentries_mean)))<ent_total; i_toy++) {
		//nentries = gRandom->Poisson(nentries_mean);
		nentries = nentries_mean;
		PlotEvents(Outfiles_names_preselected, If_Event_exist, Sample_Types, "bfcut", "(1==1)", i_toy, nentries, firstentry, 20, para_name, RWstart, RWstep,  RWnstep);
		firstentry += nentries;
	}
